...
fn get_proposal (key: u64)-> Option<Proposal> {
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal,_>>| -> Option<Proposal> p.borrow().get(&key))
}

#[ic_cdk::query]
fn get_proposal_count() -> u64 {
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal, _>>| -> u64 p.borrow().len())
}
...
fn create_proposal(key: u64, proposal: CreateProposal) -> Option<Proposal> {
    ...
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal, _>>| -> Option<Proposal> p.borrow_mut().insert(key, value))
}

#[ic_cdk::update]
fn edit_proposal (key: u64, proposal: CreateProposal) -> Result<(), VoteError> { 
  PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal, _>>| -> Result<(), VoteError> {
...
PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal,_>>| -> Result<(),VoteError> { 
    let proposal opt: Option<Proposal>= p.borrow().get(&key);
    ...
    if ic_cdk::caller() != proposal.owner {
        return Err (VoteError::Access Rejected);
    }
